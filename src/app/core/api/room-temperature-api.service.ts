...
import { RoomDetector } from '../../features/46-many-room-detectors/store/roomDetectors/room-detectors.models';

function random(max, roundDown = false) {
...
function randomBetween(min, max, roundDown = false) {
...
export class RoomTemperatureApiService {
  ...
  getAll(): Observable<RoomDetector[]> {
    ...
    const roomDetectorsList = map(ids, (id) => allRoomBaseResults[id]);
